...
const Profile = () => {
  ...
  const isLoginChangedValid = (value: string) => {
  ...
  const changeLoginInput = () => {
    setTimeout(() => loginRef.current && loginRef.current.focus(), 0);
    const value = loginRef.current && loginRef.current.value;
    setLogin(value || "");
  ...
  const resetUpdate = (evt: { preventDefault: () => void }) => {
    evt.preventDefault();
    setName(user.name);
    setNameValid(true);
    setLogin(user.email);
  ...
  const submitUpdate = (evt: FormEvent<HTMLFormElement>) => {
    evt.preventDefault();
    setLoginExist("");
